...
func InitGenerators(
	...
	// agentgateway: NodeType="agentgateway" proxies subscribe to two type URLs.
	// Dispatch keys are "<NodeType>/<typeURL>" (see findGenerator in xdsgen.go).
	// They reuse Istio's WorkloadGenerator for istio.workload.Address and have a
	// dedicated generator for the custom agentgateway.dev.resource.Resource.
	generators[string(model.Agentgateway)+"/"+v3.AddressType] = workloadGen
	generators[string(model.Agentgateway)+"/"+xds.AgentgatewayResourceType] = &xds.AgentgatewayResourceGenerator{Server: s}
