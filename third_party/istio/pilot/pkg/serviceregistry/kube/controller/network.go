...
type networkManager struct {
	...
	gatewayResourceClient kclient.Informer[*gatewayapi.Gateway]
...
func initNetworkManager(c *Controller, options Options) *networkManager {
	...
	if features.MultiNetworkGatewayAPI {
		n.gatewayResourceClient = kclient.NewDelayedInformer[*gatewayapi.Gateway](c.client, gvr.KubernetesGateway, kubetypes.StandardInformer, kubetypes.Filter{})
...
func (n *networkManager) extractGatewaysInner(svc *model.Service) bool {
	...
	for _, addr := range svc.Attributes.ClusterExternalAddresses.GetAddressesFor(n.clusterID) {
		for _, gw := range gateways {
			...
			if nodePort, exists := nodePortMap[gw.HBONEPort]; exists {
				gw.HBONEPort = nodePort
			}
	...
	if nw := svc.Attributes.Labels[label.TopologyNetwork.Name]; nw != "" {
		if gwPortStr := svc.Attributes.Labels[label.NetworkingGatewayPort.Name]; gwPortStr != "" {
			if gwPort, err := strconv.Atoi(gwPortStr); err == nil {
				gateway := model.NetworkGateway{Port: uint32(gwPort), Network: network.ID(nw)}
				if servicePortIsHBONE(svc, uint32(gwPort)) {
					gateway.HBONEPort = uint32(gwPort)
				}
				return []model.NetworkGateway{gateway}
			}
		...
		gateway := model.NetworkGateway{Port: DefaultNetworkGatewayPort, Network: network.ID(nw)}
		if servicePortIsHBONE(svc, DefaultNetworkGatewayPort) {
			gateway.HBONEPort = DefaultNetworkGatewayPort
		}
		return []model.NetworkGateway{gateway}
	}
...
func servicePortIsHBONE(svc *model.Service, port uint32) bool {
	for _, servicePort := range svc.Ports {
		if uint32(servicePort.Port) == port && (servicePort.Name == "hbone" || servicePort.Name == "hbone-mtls") {
			return true
		}
	}
	return false
...
func (n *networkManager) handleGatewayResource(_ *gatewayapi.Gateway, gw *gatewayapi.Gateway, event model.Event) error {
	...
	autoPassthrough := func(l gatewayapi.Listener) bool {
	...
	for _, addr := range gw.Spec.Addresses {
		...
		if addrType := *addr.Type; addrType != gatewayapi.IPAddressType && addrType != gatewayapi.HostnameAddressType {
			continue
		}
