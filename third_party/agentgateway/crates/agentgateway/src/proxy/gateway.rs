...
impl Gateway {
	...
	async fn serve_gateway_connect(
		...
		if let Some(socket_addr) = hbone_addr.socket_addr() {
		...
		let Some((host, port)) = hbone_addr
			.svc_hostname()
			.map(|host| (host, hbone_addr.port()))
		else {
			warn!("unsupported HBONE gateway target {hbone_addr}");
			let _ = req
				.send_response(build_response(StatusCode::BAD_REQUEST))
				.await;
			return;
		...
		let upstream_addr = resolve_gateway_hostname_target(&pi, &hostname, port);
		let upstream = match upstream_addr {
			Some(addr) => TcpStream::connect(addr).await,
			None => TcpStream::connect((hostname.as_str(), port)).await,
		};
		let Ok(mut upstream) = upstream else {
			warn!("failed to connect to HBONE gateway target {hbone_addr}");
			let _ = req
				.send_response(build_response(StatusCode::BAD_GATEWAY))
				.await;
			return;
		};
		let Ok(resp) = req.send_response(build_response(StatusCode::OK)).await else {
		...
		let mut con = agent_hbone::RWStream {
			stream: resp,
			buf: Bytes::new(),
			drain_tx: None,
		};
		if let Err(e) = tokio::io::copy_bidirectional(&mut con, &mut upstream).await {
			debug!("HBONE gateway stream closed: {e}");
		}
	}
}
...
fn resolve_gateway_hostname_target(
	pi: &Arc<ProxyInputs>,
	hostname: &str,
	port: u16,
) -> Option<SocketAddr> {
	let discovery = pi.stores.read_discovery();
	let svc = find_service_by_hostname(&discovery, hostname)?;
	for (ep, _) in svc.endpoints.iter().iter() {
		let Some(wl) = discovery.workloads.find_uid(&ep.workload_uid) else {
			continue;
		};
		let Some(ip) = wl.workload_ips.first() else {
			continue;
		};
		if wl.protocol == crate::types::discovery::InboundProtocol::HBONE {
			return Some(SocketAddr::from((*ip, 15008)));
		}
		let target_port = ep.port.get(&port).copied().unwrap_or(port);
		return Some(SocketAddr::from((*ip, target_port)));
	}
	svc
		.vips
		.iter()
		.find(|vip| vip.network == pi.cfg.network)
		.or_else(|| svc.vips.first())
		.map(|vip| SocketAddr::from((vip.address, port)))
}
