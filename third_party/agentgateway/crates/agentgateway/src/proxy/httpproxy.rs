	...
	let network_gateway = if wl.network != inputs.cfg.network {
		if let Some(gw_addr) = &wl.network_gateway {
			...
			if let Some(gw_wl) = gateway_workload {
				let gateway_identity = if gw_wl.namespace.is_empty() {
					Identity::from_parts(
						gw_wl.trust_domain.clone(),
						strng::literal!("istio-system"),
						strng::literal!("eastwest-gateway"),
					)
				} else {
					gw_wl.identity()
				};
				...
				Some((gw_addr.clone(), gateway_identity))
			} else {
				tracing::warn!(
